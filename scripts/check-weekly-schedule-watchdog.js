...
async function fetchScheduledRuns({ repository, workflowId, token, perPage = 30 }) {
    ...
    const data = await githubApiJson(
        `/repos/${owner}/${repo}/actions/workflows/${workflowId}/runs?event=schedule&branch=main&per_page=${perPage}`,
        token
    );
...
async function main() {
    ...
    const workflowId = process.env.WEEKLY_WORKFLOW_ID || '231598865';
    ...
    const runs = await fetchScheduledRuns({ repository, workflowId, token });
