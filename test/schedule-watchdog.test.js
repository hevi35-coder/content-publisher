...
test('getLatestExpectedSlotBefore resolves latest due slot with grace cutoff', () => {
    const cutoff = new Date('2026-02-17T03:30:00Z'); // Tuesday UTC
    const slot = getLatestExpectedSlotBefore(cutoff);
    assert.equal(slot.toISOString(), '2026-02-16T04:00:00.000Z'); // Monday slot
});

test('evaluateWeeklyScheduleHealth is healthy when matching schedule run exists', () => {
    const result = evaluateWeeklyScheduleHealth({
        now: new Date('2026-02-16T06:30:00Z'),
        scheduledRunTimes: ['2026-02-16T04:05:00Z'],
        graceMinutes: 120,
        earlyAllowanceMinutes: 15
    });

    assert.equal(result.status, 'HEALTHY');
    assert.equal(result.dueSlotUtc, '2026-02-16T04:00:00.000Z');
    assert.equal(result.matchedRunAtUtc, '2026-02-16T04:05:00.000Z');
});

test('evaluateWeeklyScheduleHealth reports missed when no schedule run is found', () => {
    const result = evaluateWeeklyScheduleHealth({
        now: new Date('2026-02-16T06:30:00Z'),
        scheduledRunTimes: ['2026-02-13T04:05:00Z'],
        graceMinutes: 120,
        earlyAllowanceMinutes: 15
    });
