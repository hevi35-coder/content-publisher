...
const { pushCoversToMain, shouldRequireGitSyncSuccess } = require('./git-manager');
...
async function publishToAll(draftPath, platforms = [], options = {}) {
    ...
    if (article.coverImage && !dryRun) {
        ...
        const assetSyncSuccess = pushCoversToMain(`Add cover for: ${article.title}`);
        if (!assetSyncSuccess) {
            if (shouldRequireGitSyncSuccess()) {
                throw new Error('Asset sync failed. Aborting publish to prevent broken cover URLs.');
            }
            console.warn('⚠️ [Git] Asset sync failed. Continuing because STRICT_GIT_SYNC is disabled.');
        }
