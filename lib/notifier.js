...
class Notifier {
    ...
    _buildSubject(type, step, status) {
        ...
        const prefix = process.env.DRY_RUN === 'true' ? '[DRY_RUN] ' : '';

        switch (type) {
            case 'step_complete':
                return `${prefix}${emoji} [Content Publisher] ${stepName} 완료`;
            case 'step_failed':
                return `${prefix}${emoji} [Content Publisher] ${stepName} 실패`;
            case 'pipeline_complete':
                return `${prefix}🎉 [Content Publisher] 전체 파이프라인 완료`;
            default:
                return `${prefix}[Content Publisher] ${stepName}`;
        }
    }
    ...
    _buildHtml(type, step, status, details) {
        ...
        let previewSection = '';
        if (details.webPreviewUrl) {
            previewSection = `
            <div style="margin-top: 30px; border-top: 2px dashed #ccc; padding-top: 20px; text-align: center;">
                <h3>📱 모바일에서 깨진다면?</h3>
                <p>아래 버튼을 눌러 <strong>전용 복사 페이지</strong>로 이동하세요.<br>버튼 하나로 스타일과 링크를 완벽하게 복사할 수 있습니다.</p>
                <a href="${details.webPreviewUrl}" style="display: inline-block; background: #03c75a; color: white; padding: 15px 30px; text-decoration: none; border-radius: 30px; font-weight: bold; font-size: 16px; margin-top: 10px;">
                    🌍 웹에서 열기 (복사 전용)
                </a>
            </div>`;
        }
