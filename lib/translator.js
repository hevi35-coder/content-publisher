...
async function getMarked() {
    if (!markedLoader) {
        markedLoader = import('marked').then((mod) => mod.marked || mod.default || mod);
    }
    return markedLoader;
}
...
async function adaptForPlatform(article, platform, options = {}) {
    ...
    switch (platform) {
        case 'blogger':
            ...
            // Defensive cleanup for leaked editorial notes from generation/fact-check stage.
            adapted.content = sanitizeDraftBodyContent(adapted.content);

            // Convert markdown to HTML
            adapted.content = await markdownToHtml(adapted.content);
            ...
            const { validateContent } = require('./publish-quality-gate');
            ...
            if (!validation.passed) {
                ...
                if (validation.issues.some((issue) => /Editorial notes leak/i.test(issue))) {
                    throw new Error('Editorial notes detected in adapted content. Aborting publish.');
                }
            }
